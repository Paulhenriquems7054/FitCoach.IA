            ...
            <div className="space-y-4">
               {[
                 ...
                 >
                   ...
                   {profile.gender === opt.value && <div className="bg-[#F5F1E8] text-[#1A4D2E] rounded-full p-1"><CheckIcon size={16}/></div>}
                 </button>
               ))}
            </div>
          </div>
            ...
            <div className="flex items-center justify-center py-12">
               <div className="relative w-full max-w-[200px]">
                  <input 
                    type="number"
                    value={profile.age}
                    onChange={(e) => handleChange('age', parseInt(e.target.value) || 30)}
      ...
      case 7: // Goal
        return (
           <div className="space-y-8 animate-in slide-in-from-right duration-500">
            ...
            <div className="space-y-3">
              {[
                  { val: Goal.PERDER_PESO, label: 'Perder Peso' },
                  { val: Goal.MANTER_PESO, label: 'Manter Peso' },
                  { val: Goal.GANHAR_MASSA, label: 'Ganhar Músculos' },
                  { val: Goal.MANTER_PESO, label: 'Saúde Geral' },
              ].map((g, idx) => (
                <button 
                  key={`${g.val}-${idx}`}
                ...
                >
                  ...
                  {profile.goal === g.val && <CheckIcon size={20} />}
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-6 animate-in slide-in-from-right duration-500 pb-10">
              ...
              <div>
                <label className="flex items-center gap-2 text-sm font-bold text-[#1A4D2E] mb-2 ml-2 uppercase tracking-wider">
                    <ClipboardDocumentListIcon className="w-4 h-4" /> Histórico de Saúde
                </label>
                <textarea 
